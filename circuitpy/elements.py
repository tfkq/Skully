...
class RadioButtons(SuperElement):
    ...
    def get_html(self) -> str:
        ...
        for opt in self.options:
            ...
            label = opt[1]
            if id == self.current:
                html += """<input type="radio" class="radio_button" id="{1}" name="{0}" value="{1}" checked type="submit" onchange="this.form.submit();">""".format(
                    self.id, id, label
                )
            else:
                html += """<input type="radio" class="radio_button" id="{1}" name="{0}" value="{1}" type="submit" onchange="this.form.submit();">""".format(
                    self.id, id, label
                )
